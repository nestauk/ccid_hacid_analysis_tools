...
for SOURCE_FILE in $INPUT_DIRECTORY/*.json; do
   echo "Processing solves for $SOURCE_FILE..."
   $JSON_TO_SMART_CSV \
     --source $SOURCE_FILE \
     --columns cols-cases-to-solves.json \
     --target $OUTPUT_DIRECTORY/cases-to-solves.csv \
     --mode Append
   echo "Processing users for $SOURCE_FILE..."
   $JSON_TO_SMART_CSV \
     --source $SOURCE_FILE \
     --columns cols-users-info.json \
     --target $OUTPUT_DIRECTORY/users-info.csv \
     --mode Append
done
